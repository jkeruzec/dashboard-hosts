/** Global item repository for DashboardHosts js */
if(!DashboardHosts)
{
	var DashboardHosts = {};
}
 ...
 */
DashboardHosts.JsConsole = function(id, url, historySize, confirmCommands, motd) {
	/**
	 * Id of the JsConsole
	 */
	this.id = id;
 ...
 * @private
 */
DashboardHosts.JsConsole.CSS_SELECTOR_PREFIX = "jsconsole-";
 ...
 * @private
 */
DashboardHosts.JsConsole.PARAM_COMMAND_NAME = "command";

/**
 * Confirm yes param
 * @private
 */
DashboardHosts.JsConsole.CONFIRM_YES = "y";

/**
 * Console output prefix for error message
 * @private
 */
DashboardHosts.JsConsole.OUTPUT_PREFIX_ERROR = "  ==> ";
 ...
 * @private
 */
DashboardHosts.JsConsole.PARAM_ARGS_NAME = "args";
 ...
 * @private 
 */
DashboardHosts.JsConsole.prototype.getInputEl = function() {
 ...
 * @private 
 */
DashboardHosts.JsConsole.prototype.getOutputEl = function() {
 ...
 * @private
 */
DashboardHosts.JsConsole.prototype.pushHistory = function(command) {
 ...
 * @private
 */
DashboardHosts.JsConsole.prototype.resetHistory = function() {
 ...
 */
DashboardHosts.JsConsole.prototype.getHistoryItem = function() {
 ...
 * @private
 */
DashboardHosts.JsConsole.prototype.moveCursorToTheEnd = function() {
 ...
 * @private
 */
DashboardHosts.JsConsole.prototype._init = function() {
	var jsconsole = this;
 ...
 */
DashboardHosts.JsConsole.prototype.setConsoleInput = function(input) {
 ...
 */
DashboardHosts.JsConsole.prototype.clearConsoleInput = function() {
 ...
 */
DashboardHosts.JsConsole.prototype.clearConsoleOutput = function() {
 ...
 */
DashboardHosts.JsConsole.prototype.clear = function() {
	this.clearConsoleOutput();
	this.clearConsoleInput();
};

DashboardHosts.JsConsole.prototype.writeLine = function(prefix, text, css)
{
...
DashboardHosts.JsConsole.prototype.refreshConsoleOutput = function(input, result, css)
{
	// Write input
	if (input) {
	...
	if (result) {
		...
		var prefix = css == 'command-error' ? DashboardHosts.JsConsole.OUTPUT_PREFIX_ERROR : '';
 ...
 * @private 
 */
DashboardHosts.JsConsole.prototype.parseCommandLine = function(command) {
 ...
 */
DashboardHosts.JsConsole.prototype.parseConsoleInput = function() {
	...
	if (input) {
		...
		if (input == 'clear') {
			...
			if (this.stack.length == 1) {
				...
				if (YAHOO.lang.trim(input) == DashboardHosts.JsConsole.CONFIRM_YES) {
					
					this.executeCommand(realCommand);
				}
				else {
					this.writeLine(DashboardHosts.JsConsole.OUTPUT_PREFIX_ERROR, "Command aborted!", "command-error");
					this.clearConsoleInput();
				}
				this.stack.pop();
				handleHistory = false;
...
DashboardHosts.JsConsole.prototype.hasCommandToBeConfirmed = function(command) {
 ...
 */
DashboardHosts.JsConsole.prototype.executeCommand = function(input)
{
	var jsconsole = this;
	...
	var url = this.url 
				+ "?" + DashboardHosts.JsConsole.PARAM_COMMAND_NAME + "=" + command 
				+ "&" + DashboardHosts.JsConsole.PARAM_ARGS_NAME + "=" + encodeURIComponent(args);
...
Tapestry.Initializer.jsConsoleBuilder = function(data){
	var jsConsole = new DashboardHosts.JsConsole(data.id, data.url, data.historySize, YAHOO.lang.JSON.parse(data.confirmCommands), data.motd);
	window[data.id] = jsConsole;
	jsConsole._init();
};

DashboardHosts.placeholderOldBrowserSupport = function() {
...
Tapestry.Initializer.DashboardHostsInitializer = function(data){
	// Support for placeholder with older browsers
	var placeholderSupport = new DashboardHosts.placeholderOldBrowserSupport();
};
